...
def find_cluster_dense_connection(lattice, edges, coordinate, connection_dist=1):
    ...
    assert connection_dist >= 1
    for dist in range(1, connection_dist + 1):
        for i in range(length - dist):
            equivalence.append(torch.stack([label[:, :, i], label[:, :, i + dist]], dim=2).reshape(-1, 2))  # (batch*n, 2)
            equivalence.append(torch.stack([label[:, :, i][batch_idx, edges[:, 0]],
                                            label[:, :, i + dist][batch_idx, edges[:, 1]]], dim=2).reshape(-1, 2))  # (batch*n_edge, 2)
            equivalence.append(torch.stack([label[:, :, i][batch_idx, edges[:, 1]],
                                            label[:, :, i + dist][batch_idx, edges[:, 0]]], dim=2).reshape(-1, 2))  # (batch*n_edge, 2)
